...
float4 GetMainLightColor(ToonSurfaceData tsd,float3x3 tangentTransform)
{
    ...
    float _HalfLambert_var = 0.5*dot(lerp( tsd.i.normalDir, tsd.normalDirection, _Is_NormalMapToBase ),lightDirection)+0.5;
    float4 _Set_2nd_ShadePosition_var = tex2D(_Set_2nd_ShadePosition,TRANSFORM_TEX(tsd.Set_UV0, _Set_2nd_ShadePosition));
...
float4 GetAdditionLightColor(ToonSurfaceData tsd,Light light)
{
    ...
    //v.2.0.5: 
    float3 addPassLightColor = (0.5*dot(lerp( tsd.i.normalDir, tsd.normalDirection, _Is_NormalMapToBase ), lightDirection)+0.5) * light.color.rgb * light.shadowAttenuation;
