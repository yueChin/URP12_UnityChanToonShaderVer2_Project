...
#ifndef UCTS_SHADINGGRADEMAP_TESS_URP
#define UCTS_SHADINGGRADEMAP_TESS_URP
...
void InitializeInputData(VertexOutput input, half3 normalTS, out InputData inputData)
{
    ...
    inputData.positionWS = input.posWorld;
    #endif

    half3 viewDirWS = SafeNormalize(_WorldSpaceCameraPos.xyz - input.posWorld.xyz);
    #if defined(_NORMALMAP) || defined(_DETAIL)
    float sgn = input.tangentDir.w;      // should be either +1 or -1
    float3 bitangent = sgn * cross(input.normalDir.xyz, input.tangentDir.xyz);
    inputData.normalWS = TransformTangentToWorld(normalTS, half3x3(input.tangentDir.xyz, bitangent.xyz, input.normalDir.xyz));
    #else
    inputData.normalWS = input.normalDir;
    #endif
...
float4 GetMainLightColor(ToonSurfaceData tsd,float3x3 tangentTransform)
{
    ...
    float _HalfLambert_var = 0.5*dot(lerp( tsd.i.normalDir, tsd.normalDirection, _Is_NormalMapToBase ),lightDirection)+0.5; // Half Lambert
    //v.2.0.6
    ...
    float _Specular_var = 0.5*dot(halfDirection,lerp( tsd.i.normalDir, tsd.normalDirection, _Is_NormalMapToHighColor ))+0.5; // Specular
    ...
    float _RimArea_var = (1.0 - dot(lerp( tsd.i.normalDir, tsd.normalDirection, _Is_NormalMapToRimLight ),tsd.viewDirection));
    float _RimLightPower_var = pow(_RimArea_var,exp2(lerp(3,0,_RimLight_Power)));
    float _Rimlight_InsideMask_var = saturate(lerp( (0.0 + ( (_RimLightPower_var - _RimLight_InsideMask) * (1.0 - 0.0) ) / (1.0 - _RimLight_InsideMask)), step(_RimLight_InsideMask,_RimLightPower_var), _RimLight_FeatherOff ));
    float _VertHalfLambert_var = 0.5*dot(tsd.i.normalDir,lightDirection)+0.5;
    ...
    //v.2.0.7
    half _sign_Mirror = tsd.i.mirrorFlag;
    //
    ...
    float3 viewNormal = (mul(UNITY_MATRIX_V, float4(lerp( tsd.i.normalDir, mul( _NormalMapForMatCap_var.rgb, tangentTransform ).rgb, _Is_NormalMapForMatCap ),0))).rgb;
...
//v.2.0.4
    ...
    float3 _AR_OffsetU_var = lerp(mul(UNITY_MATRIX_V, float4(tsd.i.normalDir,0)).xyz,float3(0,0,1),_AR_OffsetU);
    ...
    float2 _AR_OffsetV_var = float2(AR_VN_Rotate.x, lerp(tsd.i.uv1.y, AR_VN_Rotate.y, _AR_OffsetV));
...
//v.2.0.7
    ...
    float3 viewNormal_Emissive = (mul(UNITY_MATRIX_V, float4(tsd.i.normalDir,0))).xyz;
    ...
    float2 emissive_uv = lerp(tsd.i.uv0, _ViewCoord_UV, _Is_ViewCoord_Scroll);
    //
...
{
    float3 lightDirection = normalize(lerp(light.lightPositionWS.xyz, light.lightPositionWS.xyz - tsd.i.posWorld.xyz,light.lightPositionWS.w));
    //v.2.0.5: 
    float3 addPassLightColor = (0.5*dot(lerp( tsd.i.normalDir, tsd.normalDirection, _Is_NormalMapToBase ), lightDirection)+0.5) * light.color.rgb * light.shadowAttenuation.x;
