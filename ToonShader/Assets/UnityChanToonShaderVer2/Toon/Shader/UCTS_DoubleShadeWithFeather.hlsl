...
//
#ifndef UCTS_DOUBLESHADEWITHFEATHER_URP
#define UCTS_DOUBLESHADEWITHFEATHER_URP
...
float4 GetMainLightColor(ToonSurfaceData tsd,float3x3 tangentTransform)
{
    ...
    float _HalfLambert_var = 0.5*dot(lerp( tsd.input.normalWS, tsd.normalDirection, _Is_NormalMapToBase ),lightDirection)+0.5;
...
//v.2.0.7
    ...
    float3 NormalBlend_Emissive_Base = (mul( UNITY_MATRIX_V, float4(tsd.input.viewDirWS,0)).xyz*float3(-1,-1,1)) + float3(0,0,1);
    ...
    float2 emissive_uv = lerp(tsd.input.uv, _ViewCoord_UV, _Is_ViewCoord_Scroll);
    //
    ...
    float viewShift_var = smoothstep( 0.0, 1.0, max(0,dot(tsd.normalDirection,tsd.input.viewDirWS)));
...
//
    ...
    //v.2.0.5: Filtering the high intensity zone of PointLights
    float3 Set_LightColor = lerp(lightColor,lerp(lightColor,min(lightColor,_MainLightColor.rgb*attenuatedLightColor*_BaseColor_Step),_MainLightPosition.w),_Is_Filter_HiCutPointLightColor);
    //
